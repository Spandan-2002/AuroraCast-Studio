...
type PodcastDetailsProps = {
  params: { podcastId: Id<'podcasts'> } | Promise<{ podcastId: Id<'podcasts'> }>;
};

const resolveParams = (
  params: PodcastDetailsProps["params"]
): Promise<{ podcastId: Id<'podcasts'> }> => {
...
const PodcastDetails = ({ params }: PodcastDetailsProps) => {
  ...
  const { podcastId } = React.use(paramsPromise);
  const { user } = useUser();
